...
struct ShortCutView: View {
    var body: some View {
        ZStack {
            ...
            VStack {
                Text("上のボタンをタップすると、ショートカットアプリに 「ポンに追加」 ショートカットが作成されます。実行すると、アプリを開かずに入力画面から直接タスクを追加できます。")
                    .padding()
                    ...
                    Section(header: Text("活用方法")) {
                        ...
                        HStack(alignment: .top) {
                            ...
                            VStack(alignment: .leading) {
                                Text("背面タップで実行")
                                    .font(.title3)
                                Text("設定 - アクセシビリティ - タッチ - 背面タップに 「ポン」 ショートカットを指定すると、デバイスの背面をタップするだけでタスクを追加できます。")
                                    .font(.subheadline)
                            }
                        }
                    }
